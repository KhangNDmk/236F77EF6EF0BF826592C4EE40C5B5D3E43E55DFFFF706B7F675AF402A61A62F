...
void qua2Euler(void)
{
	// convert data from sensor frame to earth frame by using quaternion conjugate
	double Q0 = q0;
	double Q1 = -q1;
	double Q2 = -q2;
	double Q3 = -q3;
	...
	double sin_roll = 2.0*(-Q0*Q1+Q2*Q3);
	double cos_roll = 1-2*(Q1*Q1+Q2*Q2);
	
	double sin_pitch = 2*(-Q0*Q2-Q1*Q3);
	
	double sin_yaw = 2*(-Q0*Q3+Q1*Q2);
	double cos_yaw = 1-2*(Q2*Q2+Q3*Q3);
	RPY[0] = atan2(sin_roll,cos_roll)*180/PI;
	RPY[1] = asin(sin_pitch)*180/PI;
	RPY[2] = atan2(sin_yaw,cos_yaw)*180/PI;
}
